...
int main(int argc, char **argv) {
  ...
  printf("%s\n", riff.chunk_id);
  if (strcmp(riff.chunk_id, "RIFF") != 0) {
    printf("RIFF TAG not there, exiting\n");
    exit(1);
  }
  // validate_string(riff.chunk_id, "RIFF", "No valid RIFF tag, exiting");
  /*  char wave[5];
    read_string(wave, fd);
    printf("%s\n", wave);
    char fmt[5];
    ...
    now short block_align; fread(&block_align, sizeof(short), 1, fd);
